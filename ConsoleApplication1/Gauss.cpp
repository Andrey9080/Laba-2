...
public:
// конструктор будет только с параметром
...
//меняем строки местами
		...
		// переходим к диоганальному виду
		...
		for (int line=0,column=0;line < size && column == line;line++, column++)
		{
			...
			for (int i = 1; i < size; ++i)
			{
				...
				B[i] = B[i] / time_x;
				//зануляем этот столбец приводя к диогональному виду
				for (int b = 0; b < size; b++)
				{
					matrix[i][b] *= max;
					matrix[i][b] -= matrix[line][b];
				}
				//оставляем еденицу после работы над столбцом
				B[i] *= max;
				B[i] -= B[line];
				for (int b = 0; b < size; b++)
				{
